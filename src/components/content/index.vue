    ...
    <div class="flex-1">
      ...
      <div :class="['flex flex-col w-full', moduleConfig?.HasSubConfig ? 'h-1/2' : 'h-full']" ref="mainTableRef">
        ...
        <div class="flex-1 w-full ">
          ...
          <div class="flex justify-end mt-2 border-b-2 pb-2">
            <a-pagination size="small" :total="50" :show-total="total => `Total ${total} items`" show-size-changer
              show-quick-jumper />
          </div>
        </div>
      </div>
      ...
      <div class="max-h-1/2 " v-if="moduleConfig?.HasSubConfig">
        <a-tabs class="max-h-full" :tab-bar-style="{ marginBottom: '4px' }" size="small">
          <a-tab-pane v-for="subConfig in moduleConfig?.SubConfigList" :key="subConfig.ID" :tab="subConfig.TabName">
...
onMounted(async () => {
  ...
  setTimeout(() => {
    ...
    console.log("---------------------loading:" + props.moduleTab.Loading);
  }, 3000);
})
