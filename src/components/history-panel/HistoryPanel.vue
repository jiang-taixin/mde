...
// 这里最后三个字段是拼接进去的
const addOperatedColumns = (config: ModuleConfig): ModuleConfig => {
  const newConfig = JSON.parse(JSON.stringify(config));
  const baseColumn = {
    DisplayByDefault: true,
    DisplayWidth: 100,
    Hidden: false,
    IsBusinessPrimaryKey: false,
  ...
  newConfig.Attributes.push(
    { ...baseColumn, DisplayName: t('operatedBy'), Name: 'Operated By', Sequence: 10000 },
    { ...baseColumn, DisplayName: t('operatedOn'), Name: 'Operated On', Sequence: 10001 },
    { ...baseColumn, DisplayName: t('operationType'), Name: 'Operation Type', Sequence: 10002 }
  );
  return newConfig;
};
